...
for(j in 1:100){
  ...
  testing.cm<-confusionMatrix(testing_predict, test_balanced$X37_EVIDENCE)
  #Create a temporary data frame for the current iteration
  ...
  not_equal <- misclassify_train$X37_EVIDENCE != misclassify_train$predicted
  ...
  not_equal <- misclassify_test$X37_EVIDENCE != misclassify_test$predicted
  #Replace all NAs in vector to FALSE
  not_equal[is.na(not_equal)] <- FALSE
...
write.table(training_contingency, file="training_contingency.txt")
write.table(testing_contingency, file="testing_contingency.txt")
...
write.table(sorted_accuracy, file="MeanDecreaseAccuracy.txt")
write.table(sorted_gini, file="MeanDecreaseGini.txt")
```
